...
class PumpCoordinator:
    ...
    def _evaluate_heavy_cycling(self):
        if self.state.get("heavy_cycling_alerted"):
            return
        if len(self.history) < int(self.data.get(CONF_MIN_HISTORICAL_CYCLES, DEFAULT_MIN_HISTORICAL_CYCLES)):
            return
    ...
    def historical_average_power(self):
        limit = int(self.data.get(CONF_POWER_BASELINE_CYCLES, DEFAULT_POWER_BASELINE_CYCLES))
        values = [
    ...
    def expected_cycles_in_window(self) -> float | None:
        ...
        if len(usable) < int(self.data.get(CONF_MIN_HISTORICAL_CYCLES, DEFAULT_MIN_HISTORICAL_CYCLES)):
            return None
        intervals = []
    ...
    def _baseline_cycles(self):
        ...
        limit = max(
            int(self.data.get(CONF_POWER_BASELINE_CYCLES, DEFAULT_POWER_BASELINE_CYCLES)),
            int(self.data.get(CONF_MIN_HISTORICAL_CYCLES, DEFAULT_MIN_HISTORICAL_CYCLES)),
        )
