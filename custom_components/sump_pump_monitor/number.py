...
class ConfigNumber(NumberEntity):
 def __init__(self,c,key,name,lo,hi,step):
  self.c=c; self.key=key; self._attr_unique_id=f"{c.entry.entry_id}_{key}"; self._attr_name=name; self._attr_native_min_value=lo; self._attr_native_max_value=hi; self._attr_native_step=step; self._attr_device_info={"identifiers":{(DOMAIN,c.entry.entry_id)},"name":c.name,"manufacturer":"Sump Pump Monitor","model":"Sump Pump"}
 @property
