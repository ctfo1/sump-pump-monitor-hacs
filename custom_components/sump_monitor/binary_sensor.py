...
class PumpBinary(BinarySensorEntity):
 def __init__(self,c,key,name,dc,fn):
  self.c=c; self._fn=fn; self._attr_unique_id=f"{c.entry.entry_id}_{key}"; self._attr_name=name; self._attr_device_class=dc; self._attr_device_info={"identifiers":{(DOMAIN,c.entry.entry_id)},"name":c.name,"manufacturer":"Sump Monitor","model":"Sump Pump"}
 @property
